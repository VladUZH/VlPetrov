...
/**
 *
 * This class computes the "Maximal price move during scaling law", Law 3, p=2 from
 * the "Patterns in high-frequency FX data: Discovery of 12 empirical scaling laws".
 *
 * One of the results of the computation is a file with two columns: DeltaT, QuadMeanMaxPriceMove.
...
public class QuadraticMeanMaxPriceMoveScalingLaw {
     ...
     * @param numSteps
     */
    public QuadraticMeanMaxPriceMoveScalingLaw(float minDeltaT, float maxDeltaT, int numSteps){
    ...
    public void run(Price aPrice){
        ...
        for (int i = 0; i < numSteps; i++){
            while (aPrice.getTime() > endTimestamps[i]){
                ...
                sumMaxPriceMoves[i] += Math.pow(maxPriceMove, 2);
                numIntervals[i] += 1;
    ...
    public double[] finish(){
        for (int i = 0; i < numSteps; i++){
            meanMaxPriceMoves[i] = Math.pow(sumMaxPriceMoves[i] / numIntervals[i], 1/2.0);
        }
    ...
    public void saveResults(String dirName){
        ...
        try {
            ...
            String fileName = "6_quadraticMeanMaxPriceMoveScalingLaw" + "_" + dateString + ".csv";
            PrintWriter writer = new PrintWriter(dirName + "/" + fileName, "UTF-8");
            writer.println("DeltaT;QuadMeanMaxPriceMove");
            for (int i = 0; i < numSteps; i++){
