 ...
 * Created by author.
 *
 * This class is dedicated to the computation of the "Total move tick count scaling law", Law 11(tn) from
 * the "Patterns in high-frequency FX data: Discovery of 12 empirical scaling laws".
 *
 * One of the results of the computation is a file with two columns: Delta, TMticksNum.
...
public class TMtickCountScalingLaw {
     ...
     * @param numPoints
     */
    public TMtickCountScalingLaw(float lowDelta, float higDelta, int numPoints){
        ...
        ticksPerTM = new double[numPoints];
    ...
    public void run(Price aPrice){
        for (int i = 0; i < numPoints; i++){
            ...
            if (event == 1 || event == -1){
                ticksPerTM[i] += dcOses[i].getTicksTM();
                numDCs[i] += 1;
            }
        }
    }
    ...
    public double[] computeParams(){
        scalingLawParam = Tools.computeScalingParams(arrayDeltas, ticksPerTM);
    ...
    public void saveResults(String dirName){
        ...
        try {
            ...
            for (int i = 0; i < numPoints; i++){
                writer.println(arrayDeltas[i] + ";" + ticksPerTM[i]);
            }
            writer.close();
