...
/**
 *
 * This class computes the "Number of Directional-Changes scaling law", Law 0a (2) from
...
public class QuadraticMeanPriceMoveScalingLaw {
    ...
    private double[] quadMeanPriceMoves; // here the final result should be stored
     ...
     * @param numSteps
     */
    public QuadraticMeanPriceMoveScalingLaw(float minDeltaT, float maxDeltaT, int numSteps){
        ...
        quadMeanPriceMoves = new double[numSteps];
    ...
    public void run(Price aPrice){
        ...
        for (int i = 0; i < numSteps; i++){
            while (aPrice.getTime() > endTimestamps[i]){
                ...
                sumPriceMoves[i] += Math.pow(relatPriceMove, 2);
                numIntervals[i] += 1;
    ...
    public double[] finish(){
        for (int i = 0; i < numSteps; i++){
            quadMeanPriceMoves[i] = Math.pow(sumPriceMoves[i] / numIntervals[i], 1/2.0);
        }
    ...
    public double[] computeParams(){
        scalingLawParam = Tools.computeScalingParams(arrayDeltasT, quadMeanPriceMoves);
    ...
    public void saveResults(String dirName){
        ...
        try {
            ...
            String fileName = "2_quadraticMeanPriceMoveScalingLaw" + "_" + dateString + ".csv";
            PrintWriter writer = new PrintWriter(dirName + "/" + fileName, "UTF-8");
            writer.println("DeltaT;QuadraticMeanPriceMove");
            for (int i = 0; i < numSteps; i++){
                writer.println(arrayDeltasT[i] + ";" + quadMeanPriceMoves[i]);
            }
            writer.close();
    ...
    public double[] getQuadMeanPriceMoves() {
        return quadMeanPriceMoves;
    }
